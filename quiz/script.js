...
function loadQuestion(questionIndex) {
                ...
                if (currentQuestion === 0) {
                    ...
                    switch(answer.text.toLowerCase()) {
                        case 'perder peso':
                            ...
                            break;
                        case 'ficar forte':
                            userGoal = 'ganhar-peso';
                            break;
                        case 'desenvolva hábitos saudáveis':
                            userGoal = 'definir';
                            break;
                    }
    ...
    if (currentQuestion === 28) {
        ...
        const userGoal = localStorage.getItem('userGoal') || 'perder-peso';
        
        let infoMessage = '';
        let infoDescription = '';
        
        // Define a mensagem baseada no objetivo do usuário
        switch(userGoal) {
            case 'perder-peso':
                infoMessage = 'MEXA-SE: calculando sua meta de emagrecimento...';
                infoDescription = 'Um estudo da Universidade do Utah demonstrou que treinar 5 minutos por dia pode acelerar seu metabolismo, aumentar a queima de gordura e melhorar os níveis de energia.';
                break;
            case 'ganhar-peso':
                infoMessage = 'MEXA-SE: calculando sua meta de ganho muscular...';
                infoDescription = 'Pesquisas mostram que combinar exercícios de força com uma dieta adequada pode aumentar a massa muscular de forma saudável e sustentável.';
                break;
            case 'definir':
                infoMessage = 'MEXA-SE: calculando sua meta de definição...';
                infoDescription = 'Estudos comprovam que a combinação de exercícios específicos com uma dieta balanceada é a chave para alcançar a definição muscular desejada.';
                break;
            default:
                infoMessage = 'MEXA-SE: calculando sua meta...';
                infoDescription = 'Um estudo da Universidade do Utah demonstrou que treinar 5 minutos por dia pode manter o seu nível de fitness, melhorar os níveis de energia e levar a um melhor sono.';
        }
        ...
        answerOptionsElement.innerHTML = `
            <div class="height-input-container">
                ...
                <div class="imc-info">
                    ...
                    <div class="imc-info-text">
                        ${infoMessage}<br>
                        <span class="imc-description">${infoDescription}</span>
                    </div>
                </div>
            </div>
        `;
        ...
        function updateInfoMessage(targetWeight) {
            ...
            if (userGoal === 'perder-peso') {
            ...
            } else if (userGoal === 'ganhar-peso') {
                if (roundedPercentage <= 5) {
                    imcInfo.innerHTML = `
                        MEXA-SE: ganhar ${roundedPercentage}% de massa muscular<br>
                ...
                if (roundedPercentage <= 5) {
                    imcInfo.innerHTML = `
                        MEXA-SE: definir ${roundedPercentage}% do seu corpo<br>
                        <span class="imc-description">Estudos mostram que a combinação de exercícios de força com uma dieta equilibrada pode reduzir a gordura corporal e aumentar a definição muscular em 4-6 semanas.</span>
                    `;
                } else if (roundedPercentage <= 10) {
                    imcInfo.innerHTML = `
                        MEXA-SE: definir ${roundedPercentage}% do seu corpo<br>
                        <span class="imc-description">Pesquisas indicam que treinos de alta intensidade (HIIT) combinados com uma dieta rica em proteínas podem acelerar a definição muscular em até 40%.</span>
                    `;
                } else {
                    imcInfo.innerHTML = `
                        MEXA-SE: definir ${roundedPercentage}% do seu corpo<br>
                        <span class="imc-description">Um estudo com atletas mostrou que a definição muscular ideal é alcançada através de um programa progressivo que combina treino, dieta e descanso adequado.</span>
                    `;
                }
            }
        }
